...
<body>
    ...
    <ul class="dropdown-menu animated fadeInRight m-t-xs">
        ...
        <li><a href="../../waste_personnel/logout.php">Logout</a></li>
                    ...
                    <li class="active">
                        <a href=""><i class="fa fa-bar-chart-o"></i> <span class="nav-label">Reports</span></a>
                        ...
                        <div class="ibox float-e-margins">
                            <div class="ibox-content">
                                <div class="file-manager">
                                    ...
                                    <button class="btn btn-primary btn-block" onclick="exportReports('missed_pickups')">Export</button>

                                    <div class="hr-line-dashed"></div>
                                    
<div class="form-group">
    <label for="startDate">Select Start Date:</label>
    <input type="date" class="form-control" id="startDate" onchange="loadReports()">
</div>
<div class="form-group">
    <label for="endDate">Select End Date:</label>
    <input type="date" class="form-control" id="endDate" onchange="loadReports()">
</div>
    ...
    <table id="reportTable" class="footable table table-stripped toggle-arrow-tiny">
    <thead>
    <tr>
...
function loadReports() {
    var reportType = 'missed_pickups'; // Set report type to 'missed_pickups'
    ...
    if (!startDate || !endDate) {
        $.ajax({
            ...
            success: function(response) {
                ...
                if (reports.length === 0) {
                    ...
                    // Populate table body with missed pickups data
                    reports.forEach(function(report) {
                        var row = '<tr>';
                        row += '<td>' + report.id + '</td><td>' + report.area + '</td><td>' + report.address + '</td><td>'+ report.date + '</td><td>'+ report.complaint + '</td>';
                        row += '</tr>';
        ...
        $.ajax({
            url: 'load_reports.php',
            type: 'GET',
            data: { type: reportType, startDate: startDate, endDate: endDate },
            success: function(response) {
                var reports = JSON.parse(response);
                var tableBody = $('#reportTableBody');
                tableBody.empty(); // Clear existing rows
...
function exportReports(reportType) {
    var data = [];
    $('#reportTableBody tr').each(function() {
        var rowData = [];
        $(this).find('td').each(function() {
            // If the cell contains a comma, enclose it in double quotes
            var cellData = $(this).text().includes(',') ? '"' + $(this).text() + '"' : $(this).text();
            rowData.push(cellData);
        });
        data.push(rowData.join(','));
    });
    var csv = 'Missed Pickup ID, Area, Resident Address, Date of Submission, Complaint\n';
    csv += data.join('\n');

    var blob = new Blob([csv], { type: 'text/csv;charset=utf-8' });
    saveAs(blob, 'missed_pickups.csv');
}
