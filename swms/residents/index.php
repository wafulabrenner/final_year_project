...
<body>
    ...
    <ul class="dropdown-menu animated fadeInRight m-t-xs">
        ...
        <li><a href="./logout.php">Logout</a></li>
                    ...
                    <li>
                        <a href="../modules/collection_schedule/residents_collection_schedule.php"><i class="fa fa-calendar"></i> <span class="nav-label">Collection Schedule</span></a>
                        
                    </li>   
                    <li>
                        <a href="../modules/missed_pickups/missed_pickups_residents.php"><i class="fa fa-trash"></i> <span class="nav-label">Missed Pickups</span></a>
                        
                    </li>  
                     
                    <li>
                        <a href="../modules/feedback&issues/resident_issues.php"><i class="fa fa-files-o"></i> <span class="nav-label">Feedback & Issues</span></a>
    ...
    <ul class="dropdown-menu dropdown-alerts" id="pickupList">
        ...
        <li>
            <div class="text-center link-block">
                <a href="../modules/collection_schedule/residents_collection_schedule.php">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-2">
                                    <ul class="stat-list m-t-lg">
                                        <li>
                                            <h4 class="no-margins">Missed Pickups</h4>
                                            <small>This Month</small>
                                            <div class="ibox-content">
            <h1 id="missedPickupsThisMonth" class="no-margins">Loading...</h1>
       
        </div>
                                        </li>
                                        <li>
                                            <h4 class="no-margins ">Pickups</h4>
                                            <small>This Week</small>
                                            <div class="ibox-content">
            <h1 id="pickupsThisWeek" class="no-margins">Loading...</h1>
        </div>
                                        </li>
                                    </ul>
                                </div>
        ...
        <div class="ibox-content">
            <div class="table-responsive">
                <table class="table table-striped">
                    ...
                    <tbody id="upcomingScheduleTable">
                        <!-- This is where the upcoming schedule data will be displayed -->
                    </tbody>
                </table>
            </div>
        </div>
...
<div class="col-lg-6">
    <div class="ibox float-e-margins">
        <div class="ibox-content">
            <h2>Missed a pickup?</h2>
            <p>Click the button below to report a missed pickup.</p>
            <button id="reportMissedPickupBtn" class="btn btn-primary" onclick="location.href='../modules/missed_pickups/missed_pickups_residents.php';">Report Missed Pickup</button>
        </div>
    </div>
</div>
...
function fetchAndUpdateProfilePic() {
    $.ajax({
        url: 'fetch_resident_details.php', // endpoint for fetching user details
        method: 'GET',
        dataType: 'json',
...
document.getElementById('profile-link').addEventListener('click', function(e) {
    ...
    document.getElementById('profile-pic-input').addEventListener('change', function() {
        ...
        fetch('fetch_resident_pic_details.php', {
                method: 'POST',
                body: formData
            })
...
// Initialize the map
mapboxgl.accessToken = 'pk.eyJ1IjoiYnJlbm5lcm1ha2VyZXJlIiwiYSI6ImNsdWw5OGJieTB3YjEycWw1cGJ3MXN2cXYifQ.ohgHy7oGY4_M2Sq5ypA7pQ';

var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/mapbox/streets-v11',
    center: [32.5825, 0.3476], // Kampala's coordinates [longitude, latitude]
    zoom: 12
});
...
map.on('load', function () {
    map.addSource('pickup-points', {
        type: 'geojson',
        data: 'PATH_TO_YOUR_GEOJSON_FILE.geojson'
    });
    ...
    map.addLayer({
        ...
        layout: {
            'icon-image': 'marker-15', // icon image from Mapbox's style
...
// Fetch the number of missed pickups this month
$.ajax({
        url: 'fetch_missed_pickups_this_month.php',
        method: 'GET',
        dataType: 'json',
        success: function(response) {
            $('#missedPickupsThisMonth').text(response);
        },
        error: function(xhr, status, error) {
            console.error('Failed to fetch missed pickups this month:', error);
            $('#missedPickupsThisMonth').text('N/A');
        }
    });

// Fetch the number of pickups this week
$.ajax({
        url: 'fetch_pickups_this_week.php',
        method: 'GET',
        dataType: 'json',
        success: function(response) {
            $('#pickupsThisWeek').text(response);
        },
        error: function(xhr, status, error) {
            console.error('Failed to fetch pickups this week:', error);
            $('#pickupsThisWeek').text('N/A');
        }
    });


    // Fetch upcoming schedule data from the server using AJAX
    $.ajax({
        url: 'fetch_upcoming_schedule.php',
        method: 'GET',
        dataType: 'json',
        success: function(response) {
            if (response && response.length > 0) {
                var tableBody = $('#upcomingScheduleTable');
                ...
                $('#upcomingScheduleTable').html('<tr><td colspan="3">No upcoming schedule found</td></tr>');
            }
        },
        error: function(xhr, status, error) {
            console.error('Failed to fetch upcoming schedule data:', error);
            $('#upcomingScheduleTable').html('<tr><td colspan="3">Failed to fetch upcoming schedule data</td></tr>');
        }
    });
});
